...
*/
	...
	function MailPress_tracking_m006($mail)
	{
?>
...
<?php
		...
		$pathimg 	= MP_MailPress_tracking_TMP . '/mp-includes/m006/images/controlmap' . $color . '.png';
		...
		if ($tracks) foreach($tracks as $track)
		{
			...
			if ($x)
			{
				...
				if ($latitude && $longitude) $m['m006'][] = array('lat' => $latitude, 'lng' => $longitude, 'ip' => $track->ip);
			}
		} 
...
<script type='text/javascript' src='<?php echo get_option('siteurl') . '/' . MP_MailPress_tracking_PATH; ?>mp-includes/m006/js/mail.js'></script>
<script type="text/javascript">
