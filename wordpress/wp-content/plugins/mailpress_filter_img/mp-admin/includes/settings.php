...
?>
		<div id='fragment-MailPress_filter_img'>
			<div>
				<form name='filter_img_form' action='' method='post' class='mp_settings'>
					<input type='hidden' name='formname' value='filter_img_form' />
					<table class='form-table'>
						<tr valign='top'>
							<th scope='row'>
								<?php _e('&lt;img&gt; defaults','MailPress'); ?>
							</th>
							<td class='field'>
								<table>
									<tr>
										...
										<td class='nobd'>
											<input name='filter_img[align]' type='radio'<?php checked($filter_img['align'],'none'); ?>  value='none' />
						...
						<tr>
							<th scope='row'>
								<?php _e('Enter full &lt;img&gt; html tag','MailPress'); ?>  
							</th>
							<td>
								<textarea rows='5' cols='72' name='filter_img[img]'  style='font-family:Courier, "Courier New", monospace;'><?php echo apply_filters('Mailpress_input_text',$filter_img['img']); ?></textarea>
							</td>
						</tr>
<?php 
...
?>
						<tr>
							<th scope='row'>
								<?php _e('Filter result','MailPress'); ?>  
							</th>
...
<?php 
	...
	$x = MailPress_filter_img::process_img($x);
...
<?php } ?>
						<tr valign='top'>
							<th scope='row'>
								<?php _e('Keep url','MailPress'); ?>
							</th>
							<td class='field'>
								<input name='filter_img[keepurl]' type='checkbox'<?php checked($filter_img['keepurl'],'on'); ?>  id='align-none' value='on' style='margin-right:10px;' />
								<?php printf(__('NO mail attachements with site images when full url (<i>&lt;img src="<b>%1$s/...</b>"</i>) is provided.','MailPress'),get_option('siteurl')); ?>
							</td>
						</tr>
					</table>
